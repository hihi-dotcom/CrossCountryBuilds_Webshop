import { Checkbox, Card, Alert } from "flowbite-react";
import { HiTruck, HiUser, HiExclamationCircle } from "react-icons/hi";
...
export default function OrderDataModule() {
  ...
  const userData = useRouteLoaderData("root") as {
    id: number;
    role: string;
  } | null;
  ...
  const hasBicycle =
    cartItems?.some((item) => item.category === "kerékpárok") ?? false;
      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-10 text-center md:text-left">
          <h1 className="text-4xl md:text-5xl font-black text-gray-900 uppercase italic tracking-tighter">
            Rendelés{" "}
            <span className="text-blue-600 font-black">Véglegesítése</span>
          </h1>
          <p className="text-gray-500 mt-2 font-medium italic">
            Kérjük, ellenőrizze az adatait a rendelés elküldése előtt.
          </p>
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card className="border-none shadow-sm rounded-3xl overflow-hidden">
              <div className="flex items-center gap-3 mb-4 border-b border-gray-100 pb-4">
                ...
                <h3 className="text-xl font-bold uppercase italic text-gray-800">
                  Személyes adatok
                </h3>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FormField
                  ...
                  input_id="nev"
                  type="text"
                  input_placeholder="Teljes név"
                  ref={passwordref}
                />
                <p className="text-white">
                  *A regisztrációkor megadott e-mail címre küldjük rendelésed
                  összesítőjét.
                </p>
              </div>
            </Card>
            ...
            <Card className="border-none shadow-sm rounded-3xl">
              ...
              {(actionData?.errors ||
                ...
                <Alert
                  ...
                  className="rounded-2xl text-base font-bold italic"
                >
                  {actionData.ServerError ||
                    actionData.error ||
            ...
            <Card className="border-none shadow-xl bg-[#1e293b] text-white rounded-[2.5rem] sticky top-24">
              <div className="space-y-6">
                <div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="w-full flex flex-col gap-3 md:gap-5">
                      <label htmlFor={"payingmethods"} className="text-3xl">
                        {"Fizetési mód"}
                      </label>
                      <select
                        name={"paymentMethod"}
                        id={"payingmethods"}
                        className=" md:text-black border-2 md:border-transparent rounded-lg text-lg bg-transparent sm:rounded-lg px-3 py-2 md:bg-amber-50 dark-select"
                      >
                        {filtered_paying_options.map(
                          (option: {
                            value: string;
                            name: string;
                            innerText: string;
                ...
                <p className="text-[10px] text-gray-500 text-center font-medium leading-relaxed italic px-4">
                  A "Rendelés befejezése" gombra kattintva elküldi a rendelést
                  számunkra.
                </p>
              </div>
            </Card>
