...
export default function OrdersDashboard() {
  ...
  async function handleUpdateStatus(id: number, newStatus: string) {
    ...
    try {
      const result = await OrderService.UpdateOrderStat(id, {
        status: newStatus,
      });
      ...
      >
        <div className="w-full">
          <h3 className="text-xl font-bold mb-6 border-b pb-2">
            #{selectedOrderId} Megrendelt termékek
          </h3>
          <div className="space-y-2">
            {selectedOrderItems?.map((item: any) => (
              <div
                key={item.id}
                className="grid grid-cols-12 gap-2 items-center p-4 bg-white/5 rounded-xl border border-black/5 shadow-sm"
              >
            ...
            <table className="min-w-full md:w-full table-auto text-left border-collapse">
              <thead>
                <tr className="border-b-2 border-gray-100 text-gray-500 text-sm uppercase">
                  ...
                  <th className="py-3 px-2">Termékek:</th>
                  ...
                  return (
                    <>
                      <tr
                        key={s.id}
                        className={
                          s.status === "kész" ? "opacity-75 bg-green-200" : ""
                        }
                      >
                        ...
                        <td className="py-4 px-2">
                          <button
                            ...
                            className={
                              s.status === "kész"
                                ? "bg-blue-600 text-white rounded-xl py-2 px-3  flex flex-col md:flex-row items-center gap-1 transition-colors"
                                : "bg-blue-600 hover:bg-blue-800 text-white hover:font-bold py-2 px-3  flex flex-col md:flex-row items-center gap-1 transition-colors rounded-xl"
                            }
                            disabled={s.status === "kész"}
                          >
                        ...
                        <td className="py-4 px-2">
                          <select
                            ...
                            disabled={ s.status === "kész"}
                            onChange={(e) =>
                              handleUpdateStatus(s.id, e.target.value)
                            }
                          >
